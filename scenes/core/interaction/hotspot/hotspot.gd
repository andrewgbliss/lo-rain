...
func _after_ready():
	var data = GameStateStore.get_hotspot_data(object_name)
	restore(data)
...
func set_animation_state(state_name: String, value: String) -> void:
	...
	GameStateStore.set_hotspot_data(object_name, save())
...
func save() -> Dictionary:
	...
	if animation_states:
		for state_name in animation_states:
			data[state_name] = get_animation_name_by_state(animation_states[state_name])
	return data
