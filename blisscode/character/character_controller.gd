...
func _ready():
	...
	TextParser.opened.connect(_on_text_parser_opened)
	TextParser.closed.connect(_on_text_parser_closed)
	timer = Timer.new()
	timer.wait_time = .5
...
func spawn(pos: Vector2 = position):
	position = pos
	set_idle(GameManager.game_data.movement_direction)
	sprite.visible = true
	paralyzed = false
	if GameManager.game_data.movement_direction == MovementDirection.Left:
		scale.x = scale.y * -1
	elif GameManager.game_data.movement_direction == MovementDirection.Right:
		scale.x = scale.y * 1
